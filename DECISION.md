...
## 简历库：PDF 成为一等公民，可编辑简历退居为「生成器」

- **日期 / 版本**：2026-08-21，v2.31.0
- **背景**：用户要「把导出的简历、自己在别处做的简历放在同一个文件夹下统一管理，
  系统里所有需要用到简历的地方都从这里选」。旧模型是「**PDF 是某份可编辑简历的
  派生物**」——文件名里带 slug、`latest_export_for_slug` 靠 `_{slug}_` 找，
  所以**外来 PDF 结构上永远选不中**。
- **选了什么**：`data/resumes/library/` 一个文件夹装所有能往外发的 PDF，
  `library.yaml` 是每个文件的覆盖层（显示名 / 目标岗位 / 允许发送 / 来源 / 源简历）。
  可编辑简历（`data/resumes/{slug}.yaml`）保留，但它现在是**生成器**：导出一次 =
  往库里加一份成品。
- **否掉「复用 `data/resume_pdfs/exports/`」**：那里有 `_prune_exports()`，
  只留最近 20 个、多的直接 `os.remove`——**用户自己放进去的文件会被系统吃掉**。
  库里不做任何自动删除。
- **否掉「不做迁移、两套并存」**：用户明确要"整个系统级别一起改"。
  老的 `resume_matcher` 整个模块和 `resume_store` 的导出面已删除（零消费方）。
- **代价**：所有消费方一次性改完（W2 附件、W2 `match_resume`、m2 上传闸门、
  Checkpoint 1 显示、导出端点），以及它们的测试。这是"不留兼容层"必然要付的。

## 文件夹是「存在与否」的唯一真相，元数据只是覆盖层

- **日期 / 版本**：2026-08-21，v2.31.0
- **背景**：同一天的反面教材——`resumes/index.yaml` 里列着两份简历、
  对应文件却不在了，列表照样把它们显示出来，点开是空的。
- **选了什么**：库的 `list()` = **扫文件夹**，`library.yaml` 按文件名 join；
  孤儿元数据在保存时剪掉。你直接拷进文件夹的 PDF 自动出现，不需要登记。
- **为什么**：存在性和元数据分处两地、还以元数据为准，就会出现"列表里有、
  实际没有"这种状态。**以能直接观测的那个为准。**

## 「允许发送」是人工授权层，跟「能不能发」叠加而不是二选一

- **日期 / 版本**：2026-08-21，v2.31.0 / v2.31.4
- **背景**：用户 2026-08-16 提过「应该从简历库勾选允许使用的」。文件夹一旦能随便
  扔东西，随手放进去的草稿被自动发到企业系统里就是不可撤销的事故。
- **选了什么**：`allow_send` 默认**关**，要手动勾。`state`（PDF 是不是比源简历旧）
  回答「能不能发」，`allow_send` 回答「你准不准发」——**两者叠加**。
  没勾的连"最匹配的那份"都不该是它，否则等于绕过授权。
- **连带（v2.31.4）**：显式指定发哪一份（m2 的 `resume_file`、CLI `--resume`）
  **也必须是库里的一份**。原来收任意路径，一条命令就能把授权整个绕过去。
  指定哪一份是合理的逃生口，"指定一个库外的文件"不是。
- **否掉「挑不中就随便发一份」**：挑不中又没指定兜底就**拒发**。
  往企业系统里传错一份是不可撤销的，宁可不发。
- **Boss 站内简历不在此列**（用户 2026-08-21 明确）：那份文件在 Boss 的存储里，
  我们根本不提供字节。W2 的**适配版简历附件**走的是我们的文件，在库这条链上。

## 「这次发的是哪份简历」存文件名，且从交给上传器的那个值取

- **日期 / 版本**：2026-08-21，v2.32.0
- **背景**：m2 跑完想核对"刚才发的是哪份"，只能翻 run 日志——而库的内容会变、
  兜底会改、run 日志会滚动。这条记录是**事后唯一能对账的东西**。
- **选了什么**：`pending_applications.resume_file` 存**库里的文件名**。
- **否掉「存显示名」**：显示名随时可改，文件名标识的是那次真正传出去的那个字节流。
- **否掉「落库时重新 pick 一次」**：那是同一件事两份实现，而两次之间兜底/勾选
  都可能已经变了——那时"记录的"和"发出去的"就对不上，而这条记录存在的全部意义
  就是对账。改成从 `state["resume_pdf_path"]`（真正交给 `upload_file` 的那个值）
  取 basename。

## Checkpoint 2 的来源岗位：单开端点、只在选中时拉

- **日期 / 版本**：2026-08-21，v2.33.0
- **背景**：`source_job_id` 这条链（C1 → C2，1:N）一直存在，但界面从来没用过它，
  审批的人要判断"这份申请能不能提交"，判断依据却有一半在选岗页那边。
- **选了什么**：`GET /api/pending-applications/{id}/source-job`，前端在选中某条时拉。
- **否掉「把 source_job 塞进列表的 payload」**：JD 有几千字，几十条记录的列表
  每次刷新都要拖着几十份 JD，而**列表上根本不显示它**。
- **三种「没有」都是 404 且都是诚实的空**（`--job-url` 调试路径无来源岗位、
  岗位被「清掉候选」删了链会断、application 不存在）——前端据此**不显示这一块**，
  而不是显示一个空壳。

## W1/W2/W3 的步骤骨架搬到后端导出，而不是加一个跨语言对账测试

- **日期 / 版本**：2026-08-21，v2.34.0（计划 C 最后一项）
- **背景**：前端手抄的 `SKELETON` / `RUN_STEPS` / `LOOP_STEPS` 已经漂移
  （W3 少 4 步、W2 少 wechat、W1 挂着搬走的 `score_job`，详见 PITFALLS）。
- **本来可以**：照 `TestWorkflowIdContract` 的先例——那条测试读 `api/index.ts`
  把 `WorkflowId` 解析出来跟后端 `VALID_WORKFLOWS` 比，是个**跨语言对账测试**。
  同样的做法完全可以套到 SKELETON 上。
- **选了搬到后端**（`pipeline/skeleton.py` + `/api/workflow/skeleton` + 前端拉）：
  对账测试守住的是"两份别不一样"，而**两份仍然存在**；搬过去之后前端不再持有副本，
  漂移在结构上就少了一个可能。跨语言对账留给那些**必须两边都有**的东西
  （`WorkflowId` 是 TS 联合类型，前端非有不可）。
- **代价**：多一次网络请求 + 首帧空态。用模块级缓存（两个组件共用一次请求）
  和空 map 初值（调用方写的是 `run_steps[wf] ?? []`，undefined 会炸）兜住。

## 只收敛「有哪些步骤」，不收敛「每步会调哪些工具」

- **日期 / 版本**：2026-08-21，v2.34.0
- **本来可以**：把工具列表也从源码推出来，让整份骨架都correct-by-construction。
- **否掉的理由**：`send_pipeline.py` 一个文件里有 **4 个 `set_context` 和 8 个
  `_reg.call`**——哪个工具属于哪一步，正则分不出来，AST 也得靠"最近的
  set_context"这种启发式。**那种聪明本身就是新的脆弱点**：它会在某次重构后
  悄悄归错，而没有任何东西发现得了——比现在这个手抄的还糟，因为它看起来是自动的。
- **改成什么**：前端**只显示实际观测到的工具**。这不是退让——空闲态本来就
  **不知道**会调哪些工具（取决于分支），**装作知道比不显示更糟**
  （`fetch_jd` 里那个永远亮着"待运行"的 `score_job` 就是活例子）。
- **一般判据**：**收敛时不必把所有部分都收敛。** 能可靠推导的推导，
  推导不可靠的就别装作能推——退回"只报我真的知道的"。

## Checkpoint 2 的「丢弃」是删行，不是写一条 rejected

- **日期 / 版本**：2026-08-22，v2.34.1
- **背景**：Checkpoint 1 早就有「清掉这个站的候选」，Checkpoint 2 只有批准/拒绝。
  调试重跑攒下的重复记录只能手动开库删——2026-08-21 那次清理就是这么干的。
- **本来可以**：复用 `reject`。行会离开待审批队列、进「已拒绝」标签，
  从"队列清空了"这个效果看跟删掉没区别，而且零新代码。
- **否掉的理由**：**拒绝记的是一个决定**（"看过了，不投"），带 reason，是审批
  记录的一部分。调试重跑出来的重复行没人对它做过任何决定——把丢弃实现成写
  rejected，等于**往决策记录里掺进从来没发生过的决定**。以后回头看"我拒过哪些
  岗"，混进来的全是噪音。两件事就该是两个动作。
- **已批准的删不掉**（409）：批准是给 Layer 3 的提交放行信号，提交是对外发生过、
  撤不回来的事。同 `delete_pending_jobs_for_site` 保住 approved 的理由。
- **截图跟着行一起删，且必须在行删成功之后**：分两处删会留孤儿——这次清理就
  扫出 3 张没有任何行引用的截图，正是从来没人删过它们。顺序反了的话，拒绝删
  approved 的那条路径会留下"记录还在、证据没了"。两条都有测试守着，
  其中顺序那条做过变异验证（把删截图挪到 409 判断之前 → 1 红）。

## `record_open_result` 的原因枚举**取代** `form_opened`，不跟它并列

- **日期 / 版本**：2026-08-22，v2.34.2（「四个缺口」④，最后一个）
- **背景**：真机 id=72（用户已手动投过）那次，agent 正确认出页面上的「已投递」、
  正确地没去投，但结构化字段里留下的是 `form_opened=false`——**跟"根本找不到
  申请入口"一模一样**。一个是干得对，一个是干不了活儿。
- **本来可以**：加一个 `reason` 字段摆在 `form_opened` 旁边。改动更小，
  下游读 `form_opened` 的代码一行都不用动。
- **否掉的理由**：那是给同一件事留两个真相源。agent 迟早会报出
  `form_opened=true` + `reason=no_entry` 这种自相矛盾的组合，而那时没人知道该
  信哪个。本项目栽过五次「同一契约两份实现」。改成 **agent 只报 `outcome`，
  `form_opened` 变成 `@property` 从它派生**——矛盾在结构上不可能。
  测试直接验了这条：硬塞一个 `form_opened=True` 进工具，它进不了 sink。
- **`unknown` 不给 agent 用**（工具签名的 Literal 里没有它，只有模型默认值有）：
  "agent 压根没汇报"跟"agent 报了 no_entry"是两回事，把前者塞进后者是**替它编
  一个它没说过的结论**。同 `source_job_id` 那条"诚实的空"。
- **没动阶段状态**：`no_entry`/`blocked` 现在仍报 `successful`。缺口原文把这条
  记成「轻症」，要的是"结构化字段里能区分"。让它们变 `partial` 是另一个决定
  （`traced_stage` 目前只认 `truncated`＝步数耗尽，混进来会让那个词失去意义），
  留给下次。
- **prompt 与工具说明的分工**：枚举四个值的含义只写在工具 description 里；
  prompt 只补它不知道的领域知识（「已投递」这种页面长什么样、它是正确结局、
  别再硬找入口）。两边都写就是把同一份契约又抄了一遍。

## 模型下拉收敛到 provider **实例名**，不是类型

- **日期 / 版本**：2026-08-22，v2.34.3
- **背景**：`capabilities` 说类型、`available_providers` 说实例名，
  下拉两端对不上（详见 PITFALLS）。必须择一，**不能两边兼容**。
- **本来可以**：统一用**类型**（下拉只列 `ollama` / `openai_compatible` / …）。
  改动最小，`save_llm_settings` 一行都不用动。
- **否掉的理由**：类型分不出同类型的不同模型。现在 `balanced` 是
  `openai_compatible` + deepseek，将来想换成同样是 `openai_compatible` 的另一个
  模型，下拉**根本表达不出来**——而"选哪个模型"正是这个下拉唯一的用处。
- **代价（明说）**：只能在 config.yaml 里**已经配过**的 provider 之间选，
  UI 加不了新 provider。但这一直是事实（下拉本来就只列已配置的），
  以前"能塞任意类型进来"不是功能，是那个洞。
- **名字问 provider 自己要**：`_named_specs` 把 spec 构造成 provider 再读
  `.name`，不按 type+model 另算一份。重算的那份不会报错，只会在某天
  `OllamaProvider.__init__` 改了命名之后静默对不上——而这个映射是"把下拉选项
  换回配置"的唯一依据，对不上就等于存不了。有测试直接断言它与
  `router.configured_provider_names()` 一致。
- **链首是换位不是替换**：选一个已经在链里的 provider 当主力，是把它挪到链首，
  不是复制一份到链首再把原链首挤掉——链首之后那些是 fallback，
  用户选主力模型不该顺手删掉一个兜底。
- **档位列表不再手抄**：`vision` 一直在 `ModelRouter.LEVELS` 里，是端点和前端
  各写了一份三档列表，才让简历视觉解析那条链在 UI 上完全消失。现在两边都从
  后端返回的档位渲染，后端加一档 UI 自动出现（没有中文名就显示原始档位名，
  比悄悄不显示诚实）。同 v2.34.0 步骤骨架那条。

## 会话列表的岗位名：按公司名回查，但**只在该公司唯一时**才填

- **日期 / 版本**：2026-08-22，v2.34.5（用户 2026-08-16 提，本轮定的设计）
- **用户拍板的三条**：岗位名放主标题；`job_id` 为空就去找；支持按岗位筛。
- **"去找"找到什么程度，是我定的**：按公司名回查 `applications`，
  **该公司投过多个岗位时一条都不返回**。
- **本来可以**：挑一个（最近的 / 第一个）填上，覆盖率更高。
- **否掉的理由**：猜出来的岗位名会落在会话列表的**主标题**上，跟确定的
  长得一模一样——看的人没有任何办法分辨。真机那 138 条无 `job_id` 的会话里，
  22 条属于"该公司投过多个岗位"，挑一个就是 22 条错误主标题。
  **宁可显示「未关联岗位」。**
- **推测出来的要标出来**：`job_title_source` 三态（`linked` / `inferred` / `''`），
  界面上给 `inferred` 一个淡角标。同一行上两者外观相同，不标就等于宣称它是确定的。
- **拿不到岗位名时主标题退回公司名**，不留空——空的主标题比少一层信息难用得多。

## 会话列表近一半没有岗位名，不是 bug

- **日期**：2026-08-22（查证，未改）
- 真机 1170 条会话：607 条能直接拿到岗位名，**425 条 `job_id` 能 JOIN 上但
  `applications.title` 是空的**，138 条根本没有 `job_id`。
- 那 425 条是 `backfill_application_from_conversation` 造的桩行。
  它的注释写得很清楚：「title 留空、score NULL —— 作为这行是**重建的**、
  不是 W1 评分投递的天然标记」。HR 先找上门、或投递记录丢失时补的。
- **所以不是数据质量缺陷，是我们真的不知道那个岗位叫什么。**
  差点当成 bug 去"修" W1 的抓取——`applications` 空 title 逐月还在增长
  （8 月 290/693），看起来非常像回归。**判据：先看那些行的形状**——
  空 title 的 431 行全都没有 score/city/salary 却都有 hr_name，
  跟 W1 投递路径（供 title/city/salary/score、hr_name 反而常缺）正好相反，
  一眼就能看出是另一个写入者。
- **真正能提高覆盖率的线索**：那条注释同时说了「friend-list API 的 title
  没有持久化到会话上」。`getGeekFriendList` 到底返不返回岗位名，
  代码里看不出来、历史 run 日志也没留档——**需要真机抓一次**。
  若返回，持久化它能把覆盖率从 59% 拉到接近 100%，是这块最值钱的下一步。

## `jobs.db` 备份：每天一份 `VACUUM INTO`，不做写前快照、不做一键恢复

- **日期 / 版本**：2026-08-22，v2.34.6（PROGRESS 里标着「必须改（数据完整性）」）
- **背景**：唯一权威库，零备份零恢复路径。不是假想风险——历史上已因 schema
  漂移做过**三次**紧急重建（`migrate_030` / `migrate_app_rebuild` /
  `migrate_hrconv_rebuild`）。`info_pool.yaml` 早在 v2.17.1 就加了快照+回滚，
  教训一直没推广到更关键的这个库。

**① 不照抄信息池的「写前快照」。** 那边一天写几次，这边一次 W2 跑几百次写——
频率不是一个量级，写前快照在这里荒谬。改成**每天第一次打开库时**存一份，
并且放在 `_create_tables()` **之前**：那里面全是 ALTER TABLE，
而「迁移写错」正是三次紧急重建的原因，是最想防的那个场景。

**② `VACUUM INTO` 而不是复制文件。** 理由见 PITFALLS（WAL 边文件），
代价是要开一个连接、比 copy 慢，但真机 4.6 MB 的库瞬间完成。

**③ 不做一键恢复端点。** 本来可以给个按钮，跟信息池回滚对称。否掉：
服务还开着的时候换掉库文件，持有连接的线程会拿到一个半死不活的句柄；
而且**"点一下就能恢复"会让人在最慌的时候去点它**。恢复保持人工：
停后端 → 把 `data/db_snapshots/` 里的文件复制成 `data/jobs.db`。
这一条跟信息池刻意不对称，理由是「库有活连接、YAML 没有」。

**④ 保留策略抽成 `services/snapshot_retention`，信息池一并收敛过去。**
再抄一份就是同一契约两份实现（栽过五次）；而且抄错的表现特别隐蔽——
保留策略错了不报错，只在你真的需要回滚的那天发现文件已经没了。
策略本身仍是 v2.17.1 定的「最近 10 个 + 最近 14 天每天最早的各一个」。

## 主动通知走 Dashboard，且**明说它解决不了"三天没打开"**

- **日期 / 版本**：2026-08-22，v2.34.7（PROGRESS「必须改」的第二条）
- **用户拍板**：Dashboard。原话：「别的都要接 MCP 什么的，更复杂」。
- **代价必须写出来**：原始需求说的是「安静地跑好几天而无人察觉」，
  而 Dashboard 横幅**只在你打开它时才存在**。它解决的是**"打开了但没看出来
  有问题"**，不是"没打开"。把这件事含糊过去，等于交付了一个看起来解决了
  问题、实际没有的东西。
- **能做到的最远一步**：把告警数打进**浏览器标签页标题**
  （`⚠ (2) OpenJobFinder`）。后台挂着的标签也看得见，仍然零外部依赖。
  这不是"等价于推送"，只是把覆盖面从"正在看这一页"扩到"标签还开着"。

## 告警判据一律偏保守：没启用定时的 workflow 一条都不报

- **日期 / 版本**：2026-08-22，v2.34.7
- **本来可以**：只要有连续失败就报，不看有没有开定时。覆盖面更大。
- **否掉的理由**：**误报率＝存活率。** 这个项目已经在 `precommit_pii_scan`
  上学过一次——误报 → 用户 `--no-verify` → 防线归零。用户没开定时就是没打算
  让它自动跑，报「两天没成功」是纯噪音，而噪音会让人开始无视**整块横幅**，
  连真告警一起无视。宁可漏报。
  （代价是真实的：用户当前两个定时都关着，所以横幅现在恒为空。
  拿他真实的 2788 条日志假设开着算，会报出 1 条 W2 三天没成功——判据是对的，
  只是被这道闸挡住了。）
- **`skipped` 既不计数也不打断连续失败**：它是"今天撞了配额，没跑"。
  当成成功会掩盖真正的连续失败，当成失败会在正常限流时天天误报。
- **「多久没有成功过」比「失败了几次」更值钱**：后者要求失败长成某个样子，
  前者只问「上一次真的正常是什么时候」——调度器不再触发、每次都失败、
  session 过期，全都会落到这一条上。这也正是 `schedule_log` 那条老坑
  （幻影成功污染）当初最难回答的问题。

## 会话列表的岗位名到此为止：`hr_conversations.job_title` 建了又拆掉

- **日期 / 版本**：2026-08-22，v2.34.8
- **背景**：v2.34.5 把覆盖率做到 59%，剩下的靠 `backfill` 注释里那条线索——
  「friend-list API 的 title 没有持久化到会话上」。用户批准跑真机 W2 去抓。
- **抓到的事实**：API 返回 30 个字段，`title` 是 **HR 头衔**不是岗位名，
  `sourceTitle` 全空，DOM 那条路同理（详见 PITFALLS）。**这条线索是死的。**
- **本来可以**：把已经建好的列和管道留着（列 + upsert 参数 + 流水线透传 +
  6 个测试都是对的、测试全绿），等哪天找到数据源再接上。
- **否掉的理由**：**一根永远是空的列，下一个人只会以为功能坏了。**
  管道本身没有价值，有价值的是水源，而水源不存在。全部拆回去，
  重做的成本大约半小时，远小于让一列空数据长期误导人。
- **留下来的是诊断**：`extract_conversation_list` 现在会把这个 API
  **有哪些键**（只有键名，不带值）报进 run 日志。这轮真正的产出是它——
  下次 Boss 加/改/删字段时，不用再跑一趟真机才知道。
