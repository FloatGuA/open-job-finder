...
|----------|---------------------------------|
| 整体状态 | 进行中                          |
| 最后更新 | 2026-08-19（v2.27.0 m1/m2 拆图 + 死循环拦截 + run 状态如实 + 标签页工具；**join.qq.com 三轮 found=0，瓶颈定位到硬编码的 deepseek-chat**） |
| 当前版本 | 2.27.0                         |
...
### 🚧 join.qq.com（腾讯校招）三轮都跑不通，**瓶颈已定位到模型本身**（2026-08-19）

换站验证时发现的。**排查过程里我先后下过两个错误结论，都被推翻了，按顺序记下来免得重走**：

**错误结论 ①「筛选器点不动是主因」** → 只是表象。checkbox 在 a11y 树里存在但面板收起，
`click` 永远等不到它可交互；agent 点了「岗位类别」「工作城市」两个分组头，唯独没点开
「应聘项目」就去点它下面的选项。**已修**（prompt 里写清「先点分组标题展开，再点选项」）。

**错误结论 ②「岗位卡片没有 URL，m1 结构性做不到」** → 用户实测直接推翻：每张卡片点进去
都是 `https://join.qq.com/post_detail.html?postid=<id>`。我的观察（快照里 940 个岗位全是
`StaticText`、没有一个 link 节点）是对的，**推论错了**——URL 不在 agent 的视野里，不等于不存在。

**真因（已验证）**：岗位卡片是 `window.open`，详情页开在**第二个标签页**里，
而 `take_snapshot` 只返回当前选中的那一页。探针实测：

```
点 uid=1_71 'AI全栈工程师'  →  click: Successfully clicked on the element
## Pages
...
从 agent 视角：点了 → 报成功 → 再截图 → 页面一字未变 → **没有任何「发生过什么」的信号**。
而 `list_pages` / `select_page` / `close_page` 当时**全都不在 Layer 1 白名单里**。**已修**
（三个都加进两条 passthrough；它们不可能提交表单，不动 `make_guarded_click` 守的那条线）。

**修完之后第三轮仍然 found=0** —— 而这一轮的失败模式说明了真正的瓶颈：

| 轮次 | 固着的目标 | 说明 |
|---|---|---|
| 1 | `click 8_30` × 29 | 收起的 checkbox |
| 2 | `click 2_30` × 28（26 次被拦） | 同上，拦截生效但它照发不误 |
| 3 | `click 5_0` × 14（25 次被拦） | **`_0` 是 RootWebArea 根节点**，点它毫无意义 |

第三轮 122 轮里 **`list_pages` 一次都没调**，尽管工具就在它手上、prompt 里明写了怎么用。
**它连 uid 都对不上号了**（去点根节点）。

**结论：瓶颈是 `agent_runtime.build_model()` 里硬编码的 deepseek-chat，不是工具也不是 prompt。**
这条原本排在路线图后段的「LLM 模型统一配置」**其实在关键路径上**，不是清理项。
下一步建议：先把 m1 的模型换成更强的（同时把硬编码改成可配置），再谈 join.qq.com。

**顺带仍然没解的**：拦截省的是浏览器往返，省不了模型轮次——它会一直发起被拦的调用直到步数耗尽。
下一档手段是「同一调用被拦 N 次就直接终止该阶段」，把空转变成一个早点出现的明确失败。

**顺带暴露的一个设计问题**：Layer 1 白名单里没有任何输入类工具（`fill`/`type_text` 划归 Layer 3）。
但这个站有「搜索工作岗位」输入框，用搜索框比点自定义 checkbox 稳得多。「往搜索框打字」和
「往申请表里填字」风险完全不同，现在被同一条规则一起挡着——要不要为导航类输入开口子，待拍板。
