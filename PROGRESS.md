...
|----------|---------------------------------|
| 整体状态 | 进行中                          |
| 最后更新 | 2026-08-19（v2.27.0 m1/m2 拆图 + 死循环拦截 + run 状态如实；**join.qq.com 上 m1 结构性跑不通，待决策**） |
| 当前版本 | 2.27.0                         |
...
### 🚧 join.qq.com（腾讯校招）上 m1 结构性跑不通——**待用户决策**（2026-08-19）

换站验证时发现的，跟拆图/死循环都无关，是 **m1 的设计前提在这个站上不成立**。

**证据**（同一条 MCP 链路取的快照，6613 字符，与 `ensure_ready` 报告一致，
存档 `logs/joinqq_snapshot.txt`）：

1. **940 个岗位，没有一个是 `link` 或 `button`** —— 岗位卡片全是并排的 `StaticText`
   （标题、类别、事业群、工作地点各一条，无分组容器）。整张快照里的可交互节点只有
   导航栏、筛选器、页脚。而 `record_job` 需要 `url`，**快照里不存在 per-job 的 URL**。
2. 筛选器的 checkbox（如 `uid=1_30 checkbox "2027校园招聘"`）在 a11y 树里存在，
   但面板是收起的 → `click` 报 `did not become interactive`。agent 点了
   `岗位类别`/`工作城市` 两个分组头，**唯独没点开 `应聘项目`** 就去点它下面的 checkbox。
   （这一条有解：先点分组头再点选项。但解决了也拿不到岗位 URL。）

**已排除的解释**：不是没登录（快照第 17 行有个 `button "你好，<用户昵称>"` 的已登录问候，
`data/browser_profile_multisite/joinqq/` 的登录态是好的）；不是页面没渲染完
（940 个岗位的文字都在）。

**可能的出路（各有代价，需要拍板）**：
- **点 StaticText 让事件冒泡到卡片的 onClick**：卡片大概率是 React div 带 onClick，
  点它下面的文字节点可能触发跳转，再从 `navigate_page` 拿 URL。代价＝每个岗位
  一次「进详情页 + 返回」，几十个岗位就是几十个来回，60 步预算根本不够。
- **改读站点的列表 API**：join.qq.com 一定有返回岗位列表 + ID 的 JSON 接口。
  这正是 W2 从 DOM 改读 `getGeekFriendList` 的同一套路（memory `w2-getfriendlist-api`
  记为「架构升级蓝本」），一次拿全 id/标题/类别/城市，URL 可直接拼。
  **代价＝要给 Layer 1 读网络/执行脚本的能力，而 `evaluate_script` 是被安全白名单
  明确挡在外面的**（它能绕过 `make_guarded_click`）——这条动的是安全边界，必须用户点头。
- **换个岗位卡片是真链接的站点**继续验证 m1/m2，join.qq.com 单独立项。

**顺带暴露的一个通用问题**：Layer 1 的白名单里没有任何输入类工具
（`fill`/`type_text` 被列为危险、留给 Layer 3）。但 join.qq.com 有个
`textbox "搜索工作岗位"`，**用搜索框比点自定义 checkbox 稳得多**。
「往搜索框打字」和「往申请表里填字」风险完全不同，现在被一条规则一起挡住了——
要不要为导航类输入开一个口子，同样需要拍板。

**已知但还没解的**：拦截生效后 agent **仍会继续发起被拦的同一调用**（26/28 次被拦，
它照发不误），所以步数照样耗尽。拦截省的是浏览器往返，省不了模型轮次。
...
- **m1/m2 拆成两张图 + agent 死循环拦截 + run 状态不再谎报**（2026-08-19，v2.27.0，后端 1248 passed / 前端 build 绿，真机验过）
  ...
  - **真机结果（join.qq.com，两轮对照）**：第一轮 61 次工具调用里 **59 次是废动作**（29 次同参数失败 click + 30 次截图交替），`run_end` 谎报 done。第二轮拦截生效——28 次同参数点击**只有 2 次真碰浏览器**，`run_end` 如实报 `partial`。**两个修都验证有效，但都不解决 join.qq.com 本身的问题**（见下条待办）。
