...
|----------|---------------------------------|
| 整体状态 | 进行中                          |
| 最后更新 | 2026-08-03（v2.17.0.3 修跨列拖拽失效根因(dragenter 未 preventDefault) + FlowCV 式视觉层级 + 已保存简历加 A4 预览） |
| 当前版本 | 2.17.0.3                        |

## 待跟进（另开会话）

- **[2026-08-01 用户指定·未来项] 投递时按 JD 分流选简历**：现已支持多份简历（每份带「目标岗位」，`data/resumes/`），下一步让 W1 投递/W2 发简历时**按岗位 JD 自动选用对应的那份**（匹配 target 字段或 LLM 判定 → 激活对应简历/导出对应 PDF）。本轮只做了简历制作侧，分流逻辑到时再做。注意：W2 现在发的是 Boss 站内简历，接本地 PDF 附件也是这条线的一部分。
