...
# 导出存档相关的四条测试已删除（2026-08-21）：那套接口整个搬去了
# `services/resume_library.py`，意图由 `tests/test_resume_library.py` 接管。
# **`_prune_exports`（只留最近 20 个）是刻意不搬的**——用户自己放进库里的文件
# 会被它删掉，库里那条 `test_nothing_is_ever_pruned` 正是守这个。
