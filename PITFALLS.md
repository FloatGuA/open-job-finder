...
## 修掉「阻塞等待」却没改通知渠道，等于只做对一半

**现象**：系统在等人做一件事（登录、确认、插一张卡），**人不知道它在等**。
最后只留下一句「等待 XX 超时」。

**真机（2026-08-20）**：`ensure_ready` 检测到未登录后轮询等 10 分钟，提示是
`print("检测到可能未登录，请在弹出的 Chrome 窗口里手动登录…")`。用户盯着 Dashboard
看了两分半，什么都没有；run 日志整整 10 分钟只有一条 `run_start`；最后报超时。

**三个层面同时失明**：
- 提示打进 uvicorn 的 stdout —— **那里没有读者**（人看的是 Dashboard）
- `print` 没带 `flush=True`，stdout 重定向到文件时块缓冲，连翻日志都翻不到
- run 日志里没有任何记录，事后回放也看不出它当时在等什么

**特别值得记的是它的来历**：本文件早有一条「需要人工介入的脚本不能用阻塞的
`input()`」，当时的修法是改成轮询。**那一半做对了**（不再阻塞），
**但通知那一半留在了原地**——从"阻塞在没人看的终端"变成"轮询等待，提示仍在没人看的终端"。

**判据**：任何"等人做某事"的代码，问一句 **"这条提示会出现在人正在看的那个界面上吗？"**
答案是"打印到服务端日志"的，等于没有。

**正确做法**：走会推到用户界面的那条通道。本项目是 `logger.log(event, scope, data)`
——它**同时写 JSONL 和推 SSE**，于是 Dashboard 实时日志立刻显示，事后回放也看得见。
`print` 可以保留（命令行直跑时它是唯一输出），但不能是唯一出口。
（v2.28.1 已改为 `waiting_for_login` / `login_detected` 两个事件。）

## 「批准」类操作没有撤销入口，而它的下游是不可逆的

**真机（2026-08-20）**：用户在 Checkpoint 1 误批了两个岗位。
`tracker.decide_pending_job` 带 `WHERE status = 'pending'`——**approved 之后改不动**，
UI 上也没有任何撤销按钮。最后只能由维护者直接跑 SQL 改库。

那个 `WHERE` 是**刻意**的（防止对已进入填表阶段的岗位重新决策），
但它顺手把"误批了想撤回"这条合理路径也一起堵死了。

**判据**：一个状态转换如果**下游有不可逆动作**（本例：批准 → 另一个按钮把简历传进
企业的申请表），那它自己**必须可撤销**。否则人的每一次误点都是终局。

**设计要点（本例已想清楚，可复用）**：撤销**不需要**区分"下游动作发生过没有"——
只要下游用的是**差集**判断（「已填过表」= 存在 `pending_applications.source_job_id`），
撤销后重新批准仍然会被差集排除，不会二次执行。**把"做过没有"算出来而不是存成状态**，
撤销就自动安全了。

## 测试替身把错的参数名固化了——测试与代码互相印证，一起跟真实世界脱节

**现象**：单测全绿、七轮 task 评审全过，但代码里对外部工具的**每一次调用都会失败**。

**真机之前抓到（2026-08-20，计划 B 最终整支评审）**：
`executors.py` 给 chrome-devtools-mcp 的 `select_page` / `close_page` 传的是
`{"pageIdx": idx}`，而那个包声明的参数名是 **`pageId`**
（`…/chrome-devtools-mcp/build/src/tools/pages.js:36,65`，`pageId: zod.number()`）。

**为什么七轮评审都没抓到**：三个测试文件里的假工具**也是用 `pageIdx` 定义的**。
`StructuredTool.from_function` 按函数签名生成 schema，于是假工具接受 `pageIdx`、
生产代码传 `pageIdx`，**测试与代码完美互相印证**——只有真实世界不同意。

**后果链条**（评审推演，全都不报错）：
- `validate_manual` 判据③ 永远失败 → 每轮全量重探，而且**误报成"站点改版"**
- `harvest_page` 抛异常 → agent 重试 → 被防循环拦下 → **found=0**，
  正是这次重构本身要消灭的那个症状

**这是 PITFALLS 已有那条「同一外部契约散多处必漂移」的新变种**：
契约只有**一处**，但**测试替身把错的那处固化了**。旧那条的判据（"一个契约多处各写一份"）
在这里根本不成立，所以照旧判据扫是扫不出来的。

**新判据**：**假工具的参数名，必须对着真实工具的 schema 核过。**
测试替身是"外部契约的第二份声明"，跟复制一份实现同样会漂移，只是漂移的对象是
**你对外部世界的假设**，而不是你自己的代码。

**怎么核**（本次评审用的办法，成本很低）：
- 直接 grep 装好的包：`find ~/AppData/Local/npm-cache/_npx -name "*.js" -path "*<包名>*"`
- 或在会话开始时把工具 schema dump 一次（`tool.args_schema`）

**同族老坑**：spec §8 早就写过「fixture 必须用真实尺寸的快照」，动机是同一个——
**测试里的世界必须是真实世界的样子，不是你以为的样子**。那条管的是数据规模，
这条管的是接口形状。

## 「答案必须最后一次性给出」的结构会让 agent 永远不收尾——第三次犯

**这条已经在本文件/DECISION 里以两种形态出现过，2026-08-20 第三次发作。**

**历史**：
- 第 1 次（`find_jobs` 早期）：agent 每翻一页都说"让我分析当前页面的岗位"，然后又去点下一页，
  从不产出结论，一路撞到 recursion limit。
- 第 2 次：加了点击预算之后，它换成在翻页上打转。**换个地方犯同一个错。**
  当时的结论：「不是预算不够，是"答案必须最后一次性给出"这个结构本身在逼它继续探索
  （它永远觉得还没看够）」。治法是 `record_job` **随时落袋 + 每次返回进度**。
- 第 3 次（2026-08-20，`survey_structure` 首次真机）：**同一个病，新节点。**

**现象**：`survey_structure` 用了 19 个思考轮次（预算 60，**没耗尽**），全程在做正事
（用页面岗位计数验证筛选器多选/互斥：152 → 60 → 64），**最后一条是纯文本总结、
零工具调用**，一次都没调 `record_site_manual`，节点因此诚实失败。

**根因**：`record_site_manual` 的四个必填参数没有默认值
（`job_url_source` / `pagination` / `filter_interaction` / `row_split`），
**agent 必须把所有东西都搞清楚才能调第一次**。这就是"全有或全无"。

**判据（可直接套用）**：一个 agent 的产出工具，如果**必须凑齐 N 个字段才能调用一次**，
它就会一直探下去。问一句：**"它探到一半时，有没有东西可以交？"** 答案是"没有"就是这个坑。

**正确形态**（`record_job` 验证过的）：
1. 参数全部可选，探到一点报一点
2. 每次调用**返回进度**：已确定什么、还差什么
3. 齐了就明确说"完整了，可以结束"
4. 进度是 agent 的**主要导航信号**，不是日志——它同时回答"我还要不要继续"和"该找什么"

**prompt 治不了它**。本次 prompt 里明写着「不用总结成一段话，调用工具就是交付」，
agent 照样写了总结就结束。**结构问题只能用结构解决。**

## 切到新标签页 ≠ 页面渲染完了：`select_page` 不等加载，截到的是导航外壳

**现象**：m1 在 join.qq.com 上跑通了，岗位落库，`url` 各不相同且完全正确，
`jd` 各 1554 字**非空且互不相同**，分类理由写着「职责涉及 AI 相关开发」。
所有指标都正常。

**实际上**：那 1554 字里**一个字岗位内容都没有**——全是导航栏、页脚、版权声明。
拆穿它只用了一行：

```
职责 0 次 / 要求 0 次 / 岗位职责 0 次 / 任职 0 次 / 经验 0 次 / 负责 0 次
```

各条互不相同，是因为**页脚里嵌着各自的 URL**。分类 LLM 拿到这份东西照跑不误，
只能靠标题猜，还会在理由里写得像真读过 JD。

**根因**：`navigate_page` 会等页面加载完，**`select_page` 不会**。
`job_url_online` 的时序是「点击 → 新标签页 → `select_page` → 立刻 `take_snapshot`」，
截在了 SPA 渲染之前。

**修法**：切过去之后，**对刚拿到的那个 URL 再 `navigate_page` 一次**，借它
「等加载完」的语义，然后才截图。实测 4/4 一致，代价 +0.4 秒：
...
### 第一版修复：测试绿 + 变异验证通过，真机照样错

**这条比 bug 本身值钱。** 第一版不是这么修的，而是「连截几张快照，直到内容
不再变化为止」。它有完整的 TDD 流程：3 条测试、写完先看红、变异验证摘掉等待
循环后 2 条立刻变红。全量绿。**真机 52 条落库记录里只修好了 5 条，剩下 47 条照旧。**

**判据本身是错的**：**两张连续的外壳快照彼此相等，会被判成"已稳定"**，
直接返回外壳。只有当采样刚好卡在"外壳→正文"那一瞬间才会发现变化——
我的探针撞上了那一瞬（poll0=1514 → poll1=3066），于是我把一次**偶然的时序巧合**
当成了普遍规律。

**"不再变化" ≠ "渲染完了"。** 前者是可观测量，后者是我真正想要的性质，
两者之间那一步推断我没验。

**教训（可直接套用）**：
- **变异验证只证明"测试守得住这个实现"，不证明"这个实现是对的"。**
  它验的是测试与实现的一致性，判据选错时两边一起错，绿得毫无保留。
- **判据类的修复，要拿"它会不会误判"来验，而不是拿"它工作时对不对"来验。**
  正确的问法不是「稳定了是不是就渲染完了」，而是「**什么情况下没渲染完也会显得稳定**」——
  问出来立刻就能看见答案。
- **能用一个语义明确的现成机制，就别自己造判据。** `navigate_page` 的
  "等加载完"是工具作者实现并保证的；"连截几张看变没变"是我自己发明的近似，
  近似的边界得我自己负责，而我漏了最常见的那条。

---

### 三条可直接复用的判据

**① 「非空」「长度正常」「互不相同」都不能证明抓对了。**
这三个指标我全查了，全绿。**唯一拆穿它的是领域关键词计数**——招聘详情页没有
「职责」和「要求」，就是没抓到正文。**验证抓取结果要按"这类内容必然含有什么"来查，
不是按"字段有没有值"。**

**② 顺手记下的"可疑信号"不等于根因，必须实测。**
库里那批快照头部带 `busy`，我据此判定「页面还在加载」——方向对了，但
**信号是错的**：实测那张纯外壳快照 `busy=False`。同一种失败在两次观察里给出相反的
标志位。**如果照着 `busy` 写代码，测试会绿、真机会继续静默出错**，而且是靠一个
错误前提写的等待，下次谁也想不到去怀疑它。

**③ 这是本项目「验证'动作做没做'≠'结果发生没发生'」的第 N 次变体。**
...
## `index.html` 被浏览器缓存 → 前端改多少次都不生效，且伪装成「你的修复没生效」

**现象**：改了前端、构建过、后端也重启了，用户刷新页面**行为一点没变**。
更糟的是它看起来像"这个修复本身没用"——2026-08-21 真机就是这样：修好了 PDF 内联
返回、`curl` 验过响应头正确，用户那边照旧下载，我一度以为是别的原因。

**根因**：JS/CSS 的文件名带内容哈希（`index-DN_z1tMQ.js`），新构建 = 新 URL，
**那些资源本来不会拿到旧的**。但服务端返回 `index.html` 时**一个 cache 头都没带**，
浏览器按启发式缓存了它——而它引用的是**上一次构建**的哈希文件名。于是整条链锁死在
旧版本，新 JS 明明在磁盘上、也能通过 URL 取到，就是没人去请求它。

**修法**：`index.html` 返回 `Cache-Control: no-store, must-revalidate`。
带哈希的静态资源反过来——它们可以被永久缓存，URL 变了自然换新。

**一般判据**：**入口文件和内容寻址的资源，缓存策略必须相反。**
内容寻址的（带哈希）可以永久缓存；指向它们的入口一旦被缓存，
整套内容寻址的好处全部作废。

**连带教训**：我当时的验证只做到「`curl` 拿到的响应头是对的」——那只证明
**服务端行为对**，没证明**用户那边跑的是新代码**。又是一次「动作做没做 ≠
结果发生没发生」，只不过这次的"结果"发生在浏览器里。

---

## `FileResponse(..., filename=...)` 会强制下载，把内嵌预览打成下载

`filename=` 的本意只是"下载时文件叫什么名字"，**副作用是带上
`content-disposition: attachment`**，浏览器一律当下载处理。

后果不是"文件名不好看"，是**预览功能整个坏掉**：`<iframe src=...>` 请求那个 PDF
时不是渲染它，而是触发一次下载。用户看到的现象是"点一下就自动下载"，
而真相是**预览从来没成功过**。

**判据**：要在页面里显示的东西就别传 `filename=`。
要下载的另给一个入口，两个动作不要压在同一个响应上。

---

## `.catch(() => {})` 让「加载失败」和「数据为空」长得一模一样

前端拉列表时写 `.catch(() => {})`，端点 404（比如后端没重启）时页面显示"库是空的"
——**跟真的空一模一样，用户没有任何办法看出区别**。真机撞到过：文件明明在磁盘上，
页面说空的。

这是本项目反复出现的那个形状的前端版本：**「看了但没有」和「压根没看」不能长成
一个样子**（`scan_buckets` 的守门、`filter_failures` 的谎报、`set_filter_option`
的回读，全是同一件事）。

**修法**：记住错误并单独渲染。空状态和失败状态是两种状态，各画各的。

## 中转时改名，名字会泄漏到外部——企业收到的简历叫 `ojf_resume_98084.pdf`

**现象**：往企业申请表传简历，一切正常、页面显示上传成功。但对方收到的附件
**名字是一串内部编号**。HR 是会看附件名的。

**根因**：`staged_resume` 把简历复制进系统临时目录（**这一步是必要的**——
chrome-devtools-mcp 没协商 roots capability 时只允许读 OS 临时目录），
但它顺手把文件名换成了 `ojf_resume_{pid}.pdf`。而**浏览器上传时发送的就是这个
文件名**——中转产生的临时名字直接成了对方看到的东西。

**修法**：每次一个子目录 + 原文件名（`tempfile.mkdtemp()` + `src.name`）。
既唯一（两次暂存同一份不会互相踩），名字又是对的。子目录同样在临时目录之下——
**对着 chrome-devtools-mcp 的源码核过它的判定**（`McpContext.js`）：

    canonicalPath === canonicalRoot || canonicalPath.startsWith(canonicalRoot + path.sep)

不是猜的。

**一般判据**：**凡是"复制到中转位置再交出去"的路径，都要问一句「接收方会看到什么名字」。**
中转是我们的实现细节，名字却是对外可见的产物——这两件事很容易被当成一件。

**顺带**：原来的清理只 `unlink` 文件，**异常路径上目录会留下**。上传中途出错是常态
（表单没有文件控件、页面变了……），不能因此把带真实个人信息的简历留在临时目录里。
改成 `rmtree` 整个子目录，`finally` 里收。

---

## SSE 事件洪水：每条都刷新会把审批页刷爆，还会冲掉正在编辑的内容

给审批页加「run 跑完自动刷新」时，最直觉的写法是订阅 SSE、来一条刷一次。
**不行**：debug 模式下每次 `registry.call` 都发一条事件，一次 run 成百上千条。
`App.tsx` 早就为此把事件缓冲到 5Hz 才写 state（注释里写着"否则标签页会涨到几个 GB"）。

而审批页比日志更敏感：刷新会**把人正在编辑的字段值冲掉**。

**修法**：只认「这一步真的写了库」的那几个节点（`done` / `write_pending_application`），
不认工具级事件。有测试模拟 50 条工具事件、断言一次都不刷。

## 手抄的步骤骨架烂到什么程度：W3 少了一半步骤，而且不报错

「同一外部契约散多处必漂移」这条本文件早有；这里补的是**量出来的实际后果**——
2026-08-21 把前端手抄的 `SKELETON` 跟 `pipeline/**` 里的 `set_context` 对了一遍：

| | 抄的 | 代码实际 |
|---|---|---|
| W3 | 4 步（scan/locate/send/verify） | **8 步**，少了 freshness/detect/resume/upsert |
| W2 | 6 步 | 7 步，少了 wechat |
| W1 | `fetch_jd` 挂着 `score_job` | 它早搬去 `card_pipeline` 了 |

**W3 少了一半。** 而这份数据在 DECISION 里被引用过三次（每次都作为"别给 m1/m2
也写骨架"的理由），却**从没有人回头核过 W1/W2 自己那份**。

### 两种漂移方向的失败形态不一样，都不报错

- **少登记**：那一步在空闲态**根本不存在**，跑起来才冒出来。人看不出"漏了一步"，
  只会觉得"原来还有这一步"。
- **多登记**：那一步**永远显示待运行**（`fetch_jd` 里的 `score_job` 就是），
  跟"卡在这里了"长得一模一样。
...
## "夹具补个 monkeypatch" 修不掉"测试往真实用户数据里写"这一类

2026-08-15 查出 `SCHEDULE_LOG_PATH` 没被 `client` 夹具重定向，于是每跑一次
测试套件就往真实的 `data/schedule_log.jsonl` 追加调度记录——**2115/2845 条**，
而且伪装成 `duration=0` 的成功，把"上一次真的正常是什么时候"整个污染掉。

当时的修法是给夹具补一行 monkeypatch。**那只修掉了一个实例。**
2026-08-22 拿一条结构性守门去扫，当场又抓到 4 个：

| 常量 | 性质 |
|---|---|
| `SCHEDULE_CONFIG_PATH` | **可写**（`PUT /api/schedule` 存它）。潜伏——现在没有测试打这个端点，谁写第一个就静默改掉用户真实的 `data/schedule.yaml` |
| `_BOSS_DISTRICTS_PATH` / `_BOSS_POSITIONS_PATH` / `_BOSS_INDUSTRIES_PATH` | 从没用过的死 import（server.py 自己另有一份同名常量）——已删 |

### 判据

- **"漏了一个"型的 bug，补那一个不算修完。** 只要还是靠人记得在 N 个地方各写
  一行，N 就会继续增长而防线不会。要么让漏掉当场变红，要么承认它会再犯。
- **守门列的是「不许碰的位置」，不是「要打补丁的常量名」**：常量会增加，
  位置（真实 `data/`、真实 `logs/`、真实 `config.yaml`）不会。列常量名的守门
  自己就是下一个要维护的清单。
- **快照要在任何夹具打补丁之前取**（模块级，`import dashboard.server as srv`
  之后立刻取），否则拿到的是已经被重定向过的值，守门恒真。
  这条守门自带一个"它能不能变红"的用例，就是为了钉死这一点。

## `duration_seconds == 0` 不是"幻影记录"的判据

上一条那批污染记录都是 `duration=0`，于是 PROGRESS 里一度写着
「读 `schedule_log` 一律先过滤 `duration_seconds > 0`」。**污染源修掉之后，
这条建议开始藏真东西**：

- 调度器撞上 Boss 日投递上限 → `result: skipped` + `duration 0`（真没跑，且说明了原因）
- m2 简历闸门开跑前就拒绝 → `result: error` + `duration 0`（真拒绝，2026-08-20 有两条）

**判据：用"耗时短"当"没真跑"的代理指标，前提是不存在合法的快速失败——
而任何有前置闸门的流程都必然有。** 该看的是 `result` 字段。

## 复制 WAL 模式的 SQLite 文件，备份会**看起来成功**而少数据

`jobs.db` 跑在 WAL 模式（`PRAGMA journal_mode=WAL`）。已提交但尚未 checkpoint
的写在 `-wal` 边文件里，**不在主库文件里**。`shutil.copy(db_path, dest)` 只拷主
文件，于是：

- 备份文件生成了，大小正常，**能用 sqlite3 正常打开**
- 只是少了最近那些写
- 而你要到**真的去恢复的那天**才会发现

比没有备份更坏——没有备份时你知道自己没有备份。

**修法**：`conn.execute("VACUUM INTO ?", (dest,))`，一致性交给 SQLite，
而且它会顺带整理碎片（真机 4.6 MB → 3.3 MB）。

**判据（可推广）**：备份/导出类代码的测试，**必须在"源还开着、还有未落盘的写"
的状态下取一次**。只测"关掉之后拷一份"是测不到这一类的——而生产里的调用点
几乎总是开着的。这条的守门测试就是先写一条不 checkpoint 的记录再取快照。
