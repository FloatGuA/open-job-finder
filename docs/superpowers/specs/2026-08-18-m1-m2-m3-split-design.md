...
1. **m2 改名吗？** 它不填表，UI 上却叫「多站点填表」。
   候选：「勘察表单」/「读表单」/「表单侦察」。改名会跟已有心智模型冲突一次。
2. **缺口 B 选 b1 还是 b2？**（倾向 b1：m3 重扫，多出来的字段中止退回，不猜）
3. **m3 由谁触发？** Checkpoint 2 批准后自动入队，还是像 m2 那样由人再点一次
   「开始提交」？考虑到 m3 是唯一有真实提交后果的一环，倾向**人再点一次**，
   跟 v2.25.1「批准与填表解耦」同一个理由。
